...
public class Program
{
    ...
    public static void RunPatch(IPatcherState<ISkyrimMod, ISkyrimModGetter> state)
    {
        ...
        foreach (var model in models.OrderBy(m => m, StringComparer.OrdinalIgnoreCase))
        {
            ...
            foreach (var marker in markers)
            {
                ...
                {
                    if (Config.AnimatedEmissiveFade <= 0)
                    {
                        Console.WriteLine($"  ~ {model}: emissive is animated, skipped");
                        continue;
                    }
                    fade = Config.AnimatedEmissiveFade;
                    animatedFade++;
                }
    ...
    private static HashSet<string> CollectModelPaths(IPatcherState<ISkyrimMod, ISkyrimModGetter> state)
    {
        ...
        {
            // Debris keeps its meshes in a list of DebrisModel.ModelFilename
            // rather than a Model, so IModeledGetter does not reach them --
            // actors\ghostlight.nif is referenced only this way.
            if (rec is IDebrisGetter debris)
            {
                foreach (var piece in debris.Models)
                    Add(piece.ModelFilename?.ToString());
                continue;
            }
            if (rec is not IModeledGetter modeled) continue;
            Add(modeled.Model?.File.ToString());
